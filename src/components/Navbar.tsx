...
export default function Navbar() {
  ...
  return (
    <nav
      ...
      style2={{ background: '#ffffff', borderBottom: '1px solid rgba(91,53,213,0.1)' }}
    >
      <div
        style={{
          background: '#ffffff',
          borderBottom: '1px solid rgba(91,53,213,0.1)',
          position: 'fixed', top: 0, left: 0, right: 0, zIndex: 500,
        }}
      >
        <div className="max-w-7xl mx-auto px-6 lg:px-10 flex items-center justify-between"
             style={{ height: scrolled ? '64px' : '72px', transition: 'height .3s' }}>
          ...
          <div className="hidden lg:flex items-center" style={{ gap: 12 }}>
            ...
            <a href="#contact"
              style={{
                ...
                background: 'linear-gradient(135deg,#2D1580,#5B35D5)',
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            style={{
              overflow: 'hidden',
              borderTop: '1px solid rgba(91,53,213,0.1)',
              background: '#ffffff',
              position: 'fixed', top: '72px', left: 0, right: 0, zIndex: 499,
              boxShadow: '0 8px 32px rgba(91,53,213,0.08)',
            }}
            className="lg:hidden"
          >
            <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
              {LINKS.map(l => (
                <a key={l.label} href={l.href}
                   ...
                   style={{ color: '#1A0A38', textDecoration: 'none', fontWeight: 600, fontSize: 15, padding: '8px 0', borderBottom: '1px solid rgba(91,53,213,0.08)' }}>
                  {l.label}
                </a>
              ))}
              <a href="#contact"
                style={{
                  textAlign: 'center', marginTop: 8, padding: '12px 24px',
                  background: 'linear-gradient(135deg,#2D1580,#5B35D5)',
                  color: 'white', borderRadius: 10, textDecoration: 'none',
