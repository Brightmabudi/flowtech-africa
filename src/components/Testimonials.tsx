  ...
  return (
    <section ref={sectionRef} style={{ background: '#F0EDF8', padding: 'clamp(64px,8vw,112px) clamp(24px,6vw,100px)' }}>
      <div className="max-w-4xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: false }} transition={{ duration: 0.55, delay: 0.2 }}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ display: 'flex' }}>
              {['SN','AO','GV','TM','KD'].map((init, idx) => (
                <div key={init} style={{ width: 32, height: 32, borderRadius: '50%', background: 'linear-gradient(135deg,#5B35D5,#E8401A)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 800, color: 'white', border: '2px solid white', marginLeft: idx > 0 ? -8 : 0 }}>{init}</div>
              ))}
            </div>
            <div>
